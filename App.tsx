...
const App: React.FC = () => {
  ...
  // Snapshot para salvar o estado do mapa quando sair do editor
  ...
  const engineRef = useRef<{ getAudioStream: () => MediaStream | null, resetTime: () => void }>(null);
  ...
  const handleToggleSimulation = () => {
    if (gameState.isRunning) {
      ...
      // Se estiver no editor ou o jogo tiver acabado, faz o snapshot e reseta tudo
      if (isGameEnded || gameState.mode === GameMode.EDITOR || gameState.winner || gameState.players.every(p => p.vx === 0)) {
        setIsGameEnded(false);
        
        // Criamos o snapshot apenas se estivermos vindo do modo Editor
        ...
        setGameState(prev => ({
          ...
          isRunning: false, 
          winner: null,
          obstacles: JSON.parse(JSON.stringify(currentObstacles)), // Restaura obstáculos do snapshot
          players: prev.players.map(p => ({
            ...p, 
            x: p.spawnX, 
            y: p.spawnY, 
  ...
  const handleResetSimulation = () => {
    ...
    setGameState(prev => ({
      ...
      isRunning: false, 
      winner: null,
      obstacles: JSON.parse(JSON.stringify(mapSnapshot)), // Restaura fielmente do snapshot
      players: prev.players.map(p => ({
        ...p, 
        x: p.spawnX, 
        y: p.spawnY, 
  ...
  const applySpawnLayout = (x: number, y: number, layout: SpawnLayout, players: Player[]) => {
    ...
    return players.map((p, i) => {
      let nx = x, ny = y;
      if (layout === 'square') {
        nx = x + (i % 2 - 0.5) * spacing;
        ny = y + (Math.floor(i / 2) - 0.5) * spacing;
  ...
  const generateWinnableMap = () => {
    ...
    const spawnX = 225, spawnY = 80;
    const finishX = 225, finishY = 720;
    ...
    const cellW = canvasWidth / cols, cellH = (canvasHeight - 200) / rows;
    const startY = 150;

    const mainPath: {r: number, c: number}[] = [];
    let curR = 0, curC = Math.floor(cols / 2);
    while (curR < rows) {
      mainPath.push({r: curR, c: curC});
      const move = Math.random();
      if (move < 0.3 && curC > 0) curC--; else if (move < 0.6 && curC < cols - 1) curC++; else curR++;
    }

    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        if (mainPath.some(p => p.r === r && p.c === c)) continue;
        const x = c * cellW + cellW * 0.1, y = startY + r * cellH + cellH * 0.1;
        const w = cellW * 0.8, h = cellH * 0.8;
        const roll = Math.random();
        if (roll < 0.4) newObstacles.push({ id: `wall-${r}-${c}`, x, y, width: w, height: h * 0.2, type: 'wall', color: COLORS_THEME[Math.floor(Math.random() * COLORS_THEME.length)].hex });
        else if (roll < 0.55) newObstacles.push({ id: `dest-${r}-${c}`, x, y, width: w * 0.6, height: h * 0.6, type: 'destructible', health: 3, color: '#f97316' });
        else if (roll < 0.65) newObstacles.push({ id: `death-${r}-${c}`, x: x + w*0.25, y: y + h*0.25, width: w*0.5, height: h*0.5, type: 'death' });
        else if (roll < 0.72) newObstacles.push({ id: `pwr-${r}-${c}`, x: x + w*0.3, y: y + h*0.3, width: w*0.4, height: h*0.4, type: 'powerup', powerupType: Math.random() > 0.5 ? 'shrink' : 'grow' });
      }
    }
      ...
      <header className="bg-[#0f0f12] border-b border-white/5 px-6 py-3 flex justify-between items-center z-50 shadow-2xl">
        ...
        <div className="flex items-center gap-3">
          {gameState.mode === GameMode.SIMULATING ? (
            <>
              <button onClick={handleResetSimulation} className="p-3 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 transition-all flex items-center gap-2 group">
                <RotateCcw size={20} className="group-active:rotate-[-180deg] transition-transform" />
                <span className="text-[10px] font-black uppercase tracking-wider hidden sm:inline">Reset</span>
              </button>
              <button onClick={handleToggleSimulation} className={`p-4 rounded-2xl transition-all shadow-lg flex items-center gap-2 ${gameState.isRunning ? 'bg-amber-500 text-black' : 'bg-[#00f2ea] text-black'}`}>
                {gameState.isRunning ? <Pause size={24} fill="currentColor" /> : <Play size={24} fill="currentColor" />}
                <span className="text-xs font-black uppercase tracking-wider">{gameState.isRunning ? 'Pausar' : 'Iniciar'}</span>
              </button>
              <div className="h-8 w-px bg-white/10 mx-2" />
              <button onClick={() => isRecording ? (mediaRecorderRef.current?.stop(), setIsRecording(false)) : (startRecording(), handleToggleSimulation())} className={`p-4 rounded-2xl transition-all flex items-center gap-2 shadow-lg ${isRecording ? 'bg-slate-700 text-white' : 'bg-red-600 text-white shadow-red-600/20'}`}>
                {isRecording ? <VideoOff size={24} /> : <Video size={24} />}
                <span className="text-xs font-black uppercase tracking-wider">{isRecording ? 'Parar' : 'Gravar'}</span>
              </button>
            </>
          ) : (
            <div className="flex gap-2">
              ...
              <button onClick={importFollowers} disabled={!tiktokUser || isImporting} className="bg-[#00f2ea] hover:bg-[#00f2ea]/80 text-black px-6 rounded-xl text-[10px] font-black uppercase disabled:opacity-30">
        ...
        <div className="flex bg-black/50 p-1.5 rounded-2xl border border-white/5">
          <button onClick={() => {
            // Ao voltar para o editor, restaura o snapshot se houver um, senão mantém os atuais
            setGameState(s => ({ 
              ...s, 
              mode: GameMode.EDITOR, 
              isRunning: false,
              obstacles: mapSnapshot.length > 0 ? JSON.parse(JSON.stringify(mapSnapshot)) : s.obstacles
            }));
          }} className={`px-6 py-2 rounded-xl text-xs font-black transition-all ${gameState.mode === GameMode.EDITOR ? 'bg-white/10 text-white' : 'text-slate-500 hover:text-slate-300'}`}>EDITOR</button>
          
          <button onClick={() => {
            // Ao ir para simulação via botão de modo, faz snapshot se estiver saindo do editor
            if (gameState.mode === GameMode.EDITOR) {
              setMapSnapshot(JSON.parse(JSON.stringify(gameState.obstacles)));
            }
            setGameState(s => ({ ...s, mode: GameMode.SIMULATING }));
          }} className={`px-6 py-2 rounded-xl text-xs font-black transition-all ${gameState.mode === GameMode.SIMULATING ? 'bg-[#ff0050] text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}>SIMULAR</button>
        </div>
      </header>
      ...
      <div className="flex-1 flex overflow-hidden">
        {gameState.mode === GameMode.EDITOR && (
          <aside className="w-80 bg-[#0f0f12] border-r border-white/5 p-6 flex flex-col gap-6 overflow-y-auto custom-scrollbar">
            <div className="flex items-center justify-between">
              <p className="text-[10px] font-black text-[#00f2ea] uppercase tracking-widest">Builder Tools</p>
              <button onClick={generateWinnableMap} className="p-2 bg-amber-500/10 text-amber-500 border border-amber-500/20 rounded-lg hover:bg-amber-500/20"><Dices size={18} /></button>
            </div>
            ...
            {selectedObstacle && (
              <div className="pt-4 border-t border-white/5 bg-black/20 p-4 rounded-xl space-y-4 animate-in slide-in-from-right duration-200">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => { setGameState(s => ({ ...s, obstacles: s.obstacles.filter(o => o.id !== selectedId) })); setSelectedId(null); }} className="text-red-500 p-1"><Trash2 size={14}/></button>
                </div>
                
                <div className="space-y-4">
                  <div className="space-y-2">
                    <div className="flex justify-between text-[9px] font-bold uppercase text-slate-500"><span>Rotação</span><span>{selectedObstacle.rotation || 0}°</span></div>
                    <input type="range" min={0} max={360} step={45} value={selectedObstacle.rotation || 0} onChange={e => setGameState(s => ({ ...s, obstacles: s.obstacles.map(o => o.id === selectedId ? {...o, rotation: parseInt(e.target.value)} : o) }))} className="w-full accent-[#00f2ea]" />
                  </div>

                  {selectedObstacle.type === 'filling_wall' && (
                    <>
                      <div className="space-y-2">
                        <div className="flex justify-between text-[9px] font-bold uppercase text-slate-500"><span>Início (s)</span><span>{selectedObstacle.fillingStartTime || 0}s</span></div>
                        <input type="range" min={0} max={60} step={1} value={selectedObstacle.fillingStartTime || 0} onChange={e => setGameState(s => ({ ...s, obstacles: s.obstacles.map(o => o.id === selectedId ? {...o, fillingStartTime: parseInt(e.target.value)} : o) }))} className="w-full accent-[#ff0050]" />
                      </div>
                      <div className="space-y-2">
                        <div className="flex justify-between text-[9px] font-bold uppercase text-slate-500"><span>Duração (s)</span><span>{selectedObstacle.fillingDuration || 5}s</span></div>
                        <input type="range" min={1} max={30} step={1} value={selectedObstacle.fillingDuration || 5} onChange={e => setGameState(s => ({ ...s, obstacles: s.obstacles.map(o => o.id === selectedId ? {...o, fillingDuration: parseInt(e.target.value)} : o) }))} className="w-full accent-[#00f2ea]" />
                      </div>
                      <div className="space-y-2">
                        <p className="text-[9px] font-bold uppercase text-slate-500">Direção do Crescimento</p>
                        <div className="grid grid-cols-4 gap-1">
                          {['up', 'down', 'left', 'right'].map(dir => (
                            <button key={dir} onClick={() => setGameState(s => ({ ...s, obstacles: s.obstacles.map(o => o.id === selectedId ? {...o, fillingDirection: dir as any} : o) }))} 
                              className={`p-2 rounded-lg border text-[8px] font-black uppercase transition-all ${selectedObstacle.fillingDirection === dir ? 'bg-[#00f2ea] text-black border-[#00f2ea]' : 'bg-black/40 text-white/40 border-white/5'}`}>
                              {dir}
                            </button>
                          ))}
                        </div>
                  ...
                  {selectedObstacle.type === 'destructible' && (
                    <div className="space-y-2">
                      <div className="flex justify-between text-[9px] font-bold uppercase text-slate-500"><span>Vida</span><span>{selectedObstacle.health}</span></div>
                      <input type="range" min={1} max={10} step={1} value={selectedObstacle.health} onChange={e => setGameState(s => ({ ...s, obstacles: s.obstacles.map(o => o.id === selectedId ? {...o, health: parseInt(e.target.value)} : o) }))} className="w-full accent-orange-500" />
                    </div>
                  )}

                  <div className="flex flex-wrap gap-2 pt-2 border-t border-white/5">
                    {COLORS_THEME.map(c => (
                      <button key={c.hex} onClick={() => { setGameState(s => ({ ...s, obstacles: s.obstacles.map(o => o.id === selectedId ? {...o, color: c.hex} : o) })); setSelectedColor(c.hex); }} className={`w-6 h-6 rounded-md border-2 ${selectedColor === c.hex ? 'border-white' : 'border-transparent'}`} style={{ backgroundColor: c.hex }} />
                    ))}
                  </div>
                </div>
              </div>
            )}
            <button onClick={() => { if (window.confirm("Limpar todo o mapa?")) setGameState(s => ({ ...s, obstacles: [], mapSnapshot: [] })); }} className="mt-auto w-full p-4 rounded-xl bg-red-500/10 text-red-400 text-[10px] font-black uppercase border border-red-500/20 hover:bg-red-500/20 transition-all flex items-center justify-center gap-2">
        ...
        <main className="flex-1 relative bg-[#050505] flex flex-col items-center justify-center p-4">
          <SimulationEngine 
            ...
            onAddObstacle={(obs) => {
              let finalType = obs.type; let pwr = undefined;
              if (tool === 'shrink') { finalType = 'powerup'; pwr = 'shrink'; } else if (tool === 'grow') { finalType = 'powerup'; pwr = 'grow'; }
              setGameState(s => ({...s, obstacles: [...s.obstacles, { 
                ...obs, 
                type: finalType, 
                powerupType: pwr as any, 
                color: selectedColor, 
                health: finalType === 'destructible' ? 3 : 1,
                fillingStartTime: 0,
                fillingDuration: 5,
               ...
               <div className="bg-[#1a1a1e] border-2 border-[#00f2ea] p-10 rounded-[40px] shadow-2xl flex flex-col items-center gap-6">
                 ...
                 <button onClick={() => {
                   setGameState(s => ({ 
                     ...
                     isRunning: false, 
                     winner: null,
                     obstacles: JSON.parse(JSON.stringify(mapSnapshot)) // Restaura snapshot ao voltar
                   }));
                   setIsGameEnded(false);
                 }} className="w-full bg-white text-black py-4 px-10 rounded-2xl font-black uppercase hover:scale-105 transition-all">VOLTAR AO EDITOR</button>
               </div>
            </div>
          )}
        </main>
